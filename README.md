...
out/resilience.csv         4 строки — устойчивость сети при изъятии топ-N узлов (§7)
```

Три первых файла — обязательные по ТЗ. `resilience.csv` закрывает опциональный пункт.
...
## 7. Устойчивость сети: что будет, если изъять топ-N узлов
