...
Работает **офлайн**, без ключей, без GPU. Пересчёт — **16–18 секунд** (лимит ТЗ — 5 минут,
запас почти двадцатикратный). Основное время занимают 2 000 прогонов конфигурационной модели
для z-скора структурных мотивов (§5); без них пайплайн отрабатывает за полторы секунды.
...
out/resilience.csv         4 строки — устойчивость сети при изъятии топ-N узлов (§8)
...
функциях — см. [§10](#10-ai-ассистент).
...
все 63 значимые пары лежат в `out/motifs.csv` с колонками `kind`, `gid_a`, `gid_b`,
`shared`, `out_deg_a`, `out_deg_b`, `expected`, `z` — выводы проверяются вручную.
...
## 7. Как мы проверяли без разметки
...
## 8. Устойчивость сети: что будет, если изъять топ-N узлов

Опциональный пункт ТЗ — «что произойдёт с сетью при изъятии топ-N узлов, распадается ли она
на изолированные фрагменты». Для аналитика это проверка осмысленности всего топ-листа:
**если блокировка нескольких счетов рвёт сеть на куски, приоритизация нашла настоящие узлы
связности, а не просто крупные обороты.**

Цифра сама по себе ничего не значит, поэтому рядом всегда считается **изъятие такого же
числа случайных узлов** (среднее по 5 прогонам). Исходно среди 2 229 счетов, участвующих
хотя бы в одном переводе, — 16 слабосвязных компонент, крупнейшая на 1 877 узлов.
(Вместе с 19 изолированными счетами это те самые 35 компонент из §5.)

| Изъято | Компонент | Случайно | Крупнейшая | Случайно | Одиночек |
|---|---|---|---|---|---|
| **5** | **171** | 16.6 | 1 616 | 1 872.4 | 117 |
| 10 | 212 | 27.2 | 1 549 | 1 849.6 | 151 |
| 20 | 323 | 25.6 | 1 361 | 1 847.2 | 219 |
| 50 | 588 | 58.6 | 1 088 | 1 776.2 | 445 |

**Пять узлов из топа рвут сеть на 171 компоненту. Пять случайных — на 16.6,
то есть практически не меняют ничего.** Разрыв в десять раз, и это на пяти счетах
из 2 248. Крупнейшая компонента при этом теряет 261 узел, а 117 счетов остаются
в полной изоляции — деньги до них больше не доходят ни по какому пути.

Результат лежит в `out/resilience.csv`, считается в том же прогоне `run.py`.

**Как это читать.** Это **не** рекомендация блокировать счета — такого вывода данные
не поддерживают. Это ответ на вопрос «правильные ли узлы выбрал приоритет»: узлы,
через которые проходит связность сети, и узлы с максимальным `priority_score` — одни и те же.
Композит приоритета про связность ничего не знает, он собран из потоков и происхождения денег,
поэтому совпадение — независимая проверка, а не тавтология.
...
| `network_resilience(removed)` | что будет с сетью при изъятии топ-N узлов (§8) |
